...
- ✅ **Port MIDI de retour `OpenLamp Feedback`** (moteur → DAW) : `feedback: true` → port MIDI OUT échoant l'état en langage OpenLamp MIDI (looks exclusifs, util on/off, CC 1-8). **+ état RÉEL** : `feedback_wled: {host, channel}` s'abonne au WebSocket WLED (`ws://<host>/ws`) et reflète l'état ACTUEL du device (on/off→util, bri→CC1, couleur→CC3/4, fx→CC5), donc aussi les **changements externes** (app WLED, toggle physique). Mapping état→MIDI self-testé (émet seulement sur changement). Besoin : `pip install websocket-client`. Le rendu riche (nom d'effet) reste Tier 2 (M4L).

## See also

- Les tâches runtime de `midi.py` (test end-to-end sur vraies lampes, rate-limit du beat pulse via UDP realtime, test MPE, Link refinement, MIDI 2.0) sont suivies dans [openlamp/openlamp-spec-midi TASKS.md](https://github.com/openlamp/openlamp-spec-midi/blob/main/TASKS.md) — le moteur est l'implémentation de référence de cette convention.
